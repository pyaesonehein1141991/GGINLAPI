...
public class TownShipCoeSchedular {
	...
	//	List<Agent> agentList = agentService.findAll();
		...
		if(dataList.size()>0) {
			...
			File agentsFile = new File("TownShipCode.csv");
...
//			writesCsvFromBean(Paths.get(agentsFile.getPath()),agentList);
			...
			FileOutputStream fos = new FileOutputStream("TownShipCode.zip");
			...
			File checksumFile = new File("TownShipCodeChecksum".concat(".md5"));
			
			objectMapper.writeValue(checksumFile,checksum);
			String tempDir= fileDir.concat(":\\AceSharedFolder\\TownShipCode").concat(FileService.getDateToString(new Date()));
			
			Path filePath = Paths.get(tempDir.concat("\\TownShipCode.zip"));
			Files.createDirectories(filePath.getParent());
			
			Files.move(Paths.get(toCheckSumFile.getPath()),Paths.get(tempDir.concat("\\TownShipCode.zip")),StandardCopyOption.REPLACE_EXISTING);
			Files.move(Paths.get(checksumFile.getPath()),Paths.get(tempDir.concat("\\TownShipCodeChecksum.md5")),StandardCopyOption.REPLACE_EXISTING);
